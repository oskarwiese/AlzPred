...
def subject_images(sub):
	...
	for z in range(0,256):
            ...
            if img_xy.sum() != 0:
                io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/data/no_black/xy/train/A/A/{sub}_xy_z_{z}_15T.png', img_xy, check_contrast=False)
            img_xz = np.array(img[:,z,:]).astype(np.uint8)
            if img_xz.sum() != 0:
                io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/data/no_black/xz/train/A/A/{sub}_xz_y_{z}_15T.png', img_xz, check_contrast=False)
            img_yz = np.array(img[z,:,:]).astype(np.uint8)
            if img_yz.sum() != 0:
                io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/data/no_black/yz/train/A/A/{sub}_yz_x_{z}_15T.png', img_yz, check_contrast=False)

if __name__ == "__main__":

        li = []
        f = open("/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/preprocessing_scripts/slicing_scripts/1.5T_imgs/ids_ADNI1_15T.txt", "r")
        for x in f:
            li.append(x.strip())
        f.close()
