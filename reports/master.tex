...
Now, EM (GCA) Registration is done. This computes and transforms to alugb %TODO: hva?
...
%TODO: Nævn noget om denne figur, den ankommer meget pludseligt
\begin{figure}[H]
	\centering
...
<<<<<<< HEAD
	\caption{The general autoencoder architecture. A kernel does convolutions to reduce and increase the image dimensions. The smaller the dimensions, the larger the feature complexity. The code between the encoder and decoder aaa. Figure courtesy of \cite{autoencoder}.} %TODO fix
=======
	\caption{The general autoencoder architecture. A kernel does convolutions to reduce and increase the image dimensions. The smaller the dimensions, the larger the feature complexity. The code between the encoder and decoder aaa. Figure courtesy of \cite{autoencoder}.} %TODO: Find ud af hvad resnet blocks gør
>>>>>>> b89c94831004862ebb955ddb8b0175db8012c587
...
%TODO: opdatér denne tabel til at have alle modellernes hyperparametre
\begin{table}[H]
	\centering
	...
	\end{tabular}
\end{table} 
%TODO: Dette passer ikke længere
...
%TODO: Disse skal nok være en del mindre
\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.7\textwidth}
		\centering
...
%TODO: Skriv sektion om classifier. Overvej om første subsection skal med herned
...
%TODO: Endten mere info eller smid det hele i classifier-sektionen.
...
\subsubsection{Deep Neural Network Model}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth, trim={0 0 69cm 0},clip]{imgs/convolution}
	...
	The kernels in a convolutional neural network are learnable parameters. The dimension of the kernels are smaller than the input, which in turn will reduce the dimension of the input image. All padding are set to zero, which also can be seen in the source code %TODO ref soruce.  
	...
	Following table will outline the architecture of the model: 
	...
	\begin{table}[H]
		...
		\begin{tabular}{l l l l l l l l l}
			\toprule
			& \textbf{Layer \& Type}           &&&&& & \textbf{In channel, Out channel, Kernel Size, Stride}    & \\ \midrule
			& Layer1: Conv3d        &&&&& & (1, 32, 3, 1)& \\
			& Layer2: Conv3d        &&&&& & (32, 64, 3, 2)& \\
			& Layer3: Conv3d        &&&&& & (64, 128, 3, 1)& \\
			& Layer4: Conv3d        &&&&& & (128, 256, 3, 2)& \\
			& Layer5: Conv3d        &&&&& & (256, 512, 5, 1)& \\
			& Layer6: Conv3d        &&&&& & (512, 1024, 3, 2)& \\
			\toprule
			&       &&&&& & \textbf{In channel, Out channel}  & \\ \midrule
			& Layer7: Linear        &&&&& & (4415488, 128)& \\
			& Layer8: Relu        &&&&& & & \\
			& Layer9: LayerNorm        &&&&& & (128,128) & \\
			& Layer10: Dropout        &&&&& & & \\
			& Layer11: Linear        &&&&& & (128, 2)& \\
		\end{tabular}
	\end{table}
...
%TODO: Skriv resten af denne sektion
...
%TODO: NÅR VI TRÆKKER GROUND TRUTH OG GENERERET BILLEDE FRA HINANDEN SKAL DE MÅSKE REGISTRERES TIL SAMME SPACE FØRST
...
%TODO: Omskriv så det passer til nuværende rapportstruktur.
...
%TODO: Der skal være en figur her med de genererede all_billeder


\subsection{XY RMSE Calculations}
%TODO:
%TODO: Dette skal være tabel over gennemsnitlige RMSE-værdier og geometric accuracies for alle modellerne.
%TODO:
\begin{table}[H]
	\begin{center}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Respectively, the generator, discriminator, cycle-consistency and identity losses. The model was run for 140 epochs, since this was the maximum amount of epochs the model was allowed to run on the HPC.}
	\label{fig:horse2zebra_losses}
\end{figure}

\subsubsection{Progress of Generating Horses}
\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.8\textwidth}
		\centering
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Images reconstructed by running the chosen sample images through the horse2zebra CycleGAN. The first column contains the original image, second image shows the generator having converted the image the the opposite class and the third image contains the cycle-consistency image, where the original class is reconstructed from the second image. This aims to show how well the CycleGAN generates images and converts them back again. \textbf{Right three columns:} Horses converted to zebras and back to horses again. \textbf{Left three columns:} Zebras converted to horses and back.}
	\label{fig:norse2zebra_cycle_consistency}
\end{figure}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Images run through the "wrong" model such that horses are generated from horses and zebras are generated from zebras to analyze how well the model avoids modifying images that are already part of the target class. The first column of each class contains the real image and the second column shows the generated image after applying the generator that generates the same class. \textbf{Right two columns:} A horse image generated into a horse image. \textbf{Left two columns:} A zebra image generated to zebra image.}
	\label{fig:horse2zebra_identity}
\end{figure}
