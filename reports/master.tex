...
%\usepackage{program}
%\title{CycleGAN (Cyclical Generative Adversarial Network) for Field Strength Bias Removal in Alzheimer's Disease Classification Model}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Comparison of troubleshooting efforts from the different methods of decoding in the generators.}
\end{figure}
...
The slight lines in the generated images after using bilinear interpolation for upsampling are not nearly as problematic but are still unfortunate as a final output. To solve this, we implemented a kernel size that was divisible by the stride in the deconvolution layers, as mentioned in the previous paragraph. This should make the kernel spread information from each pixel more evenly between new pixels and supply a less grainy final image \cite{checkerboard}. In illustration \ref{fig:troubleshootc}, most of the noise has finally been removed and no lines persist in the image. The full comparison between results after each improvement has been implemented, can be seen for the same subject in figure \ref{fig:troubleshoot}.
...
\end{figure}
Even after the struggle of implementing all of the these changes, some challenges are still present when generating images, though at this point the severity of the problems has been significantly reduced. It turns out that in the very rare case of non-zero pixels approaching the image border, these pixels get sliced in a weird fashion, which also creates some lined artifacts. This artifact can be seen in figure \ref{fig:troubleshoot_verticals} for a random subject for each field-strength, where the above explained case manifested itself.

\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.7\textwidth}
		\centering
		\includegraphics[width=0.475\linewidth]{imgs/1.5T_border_artifact}%
		\hfill
		\includegraphics[width=0.475\linewidth]{imgs/3T_border_artifact}
	\end{subfigure}
	\caption{Two generated images before training was completed (half an epoch and one and a half epochs respectively). The left image is 1.5T and the right is 3T. The image artifact occurs on the left side of the images, where a trading pattern of low-intensity and high-intensity pixels can be seen forming small vertical lines.}
	\label{fig:troubleshoot_verticals}
\end{figure}
Since this image artifact occurs so rarely in generated images, it has been decided that resolving this issue will be left as future work. For future reference, the problem likely transpires because of padding and not kernel size nor stride, since these hyperparameters where debugged earlier in this section. A likely cause is the padding adding extra pixels to the edges of images which means that these pixels are not as likely to learn important information in the image creation, since they are only a part of the generator network for a very short amount of time.
