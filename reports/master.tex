...
Of just as much importance as the patch-GAN architecture and general model interactions is the layout of each layer and the functions used throughout the discriminator network. A general overview of the layers, kernel size, type of convolution, stride and padding is shown below in figure \ref{fig:cycleganlayers}.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{imgs/cyclegan_layers}
	\caption{The primary model layout consisting of the input, layers and output. The input is the grey-scale intensity-based MRI input of size 256x256x1. The five consecutive convolutional layers each have a specified kernel size, stride and padding as noted in the figure. The output is constructed from running the patchGAN on the last layer of the model to get a 30x30x1 matrix of values between 0 and 1. The specific values and layer sizes are specifed in table }%TODO:
	\label{fig:cycleganlayers}
\end{figure}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|r|}
		...
		Input                         & 3x256x256       \\ \hline
		64x4x4 conv, stride 2, pad 1  & 64x128x128      \\ \hline
		128x4x4 conv, stride 2, pad 1 & 128x64x64       \\ \hline
		256x4x4 conv, stride 2, pad 1 & 256x32x32       \\ \hline
		512x4x4 conv, stride 1, pad 1 & 512x31x31       \\ \hline
		1x4x4 conv, stride 1, pad 1   & 1x30x30         \\ \hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|r|}
		...
		Layer                                            & Activation Size \\ \hline
		Input                                            & 3x256x256       \\ \hline
		64x7x7 conv, stride 1, pad 3                     & 64x256x256      \\ \hline
		128x3x3 conv, stride 2, pad 1                    & 128x128x128     \\ \hline
		256x3x3 conv, stride 2, pad 1                    & 256x64x64       \\ \hline
		9 residual blocks                                & 256x64x64       \\ \hline
		128x3x3 convTranspose, stride 2, pad 1, outpad 1 & 128x128x128     \\ \hline
		64x3x3 convTranspose, stride 2, pad 1, outpad 1  & 64x256x256      \\ \hline
		3x7x7 conv, stride 1, pad 3                      & 3x256x256       \\ \hline
	\end{tabular}
\end{table}
