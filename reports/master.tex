...
More specifically, this project will only use MRI data from ADNI1 and ADNI2. 
...
%TODO tilføj sektion om shellscripts og struktur af mapper
...
For this section, a number of notations will be introduced. Since the goal is to map between the two domains $X$ and $Y$, we introduce $G : X \rightarrow Y$ and $F : Y \rightarrow X$. Furthermore, $y \sim p_{\text {dau }}(y)$ and $x \sim p_{\text {data }}(x)$ refer to the distributions of each domain and $x$ and $y$ refer to samples from each domain. Lastly, the discriminators for each domain are given by $D_X$ and $D_Y$.
...
\subsubsection{cycleGAN for mapping between 1.5T to 3T (ANDET NAVN)}
Two different methods for removing bias in the original classifier "CITE KAMILLA" will be implemented in this project. More specifically to investigate whether the mapping between the 1.5T and 3T domain can be used to remove bias in a classifier the following approaches will be demonstrated.

\subsubsection{cycleGAN: XY-, XZ-, and YZ-Planes } 
One of the approaches to map between the 1.5T and 3T domain was to train three different cycleGANs on three different planes, namely the: XY-, XZ- and YZ-Planes, to reconstruct the original 3D image. For the training data one dimension is held constant, thus 256 images are constructed for each dimension, X,Y and Z. All the images where the sum of the pixel values are equal to zero are removed, ensuring that images in the training dataset is are not all black. Each of these cycleGAN will be referred to as: \texttt{cycleGAN\_XY}, \texttt{cycleGAN\_XZ} and \texttt{cycleGAN\_YZ}, where the not mentioned dimension is the one held constant. 

\subsubsection{cycleGAN: sliced 3D image }

The second approach is to train one unified cycleGAN on all of the slices from the XY, XZ and YZ planes dependently.
 

\subsubsection{Classifier - OSKAR} 
...
%TODO insert loss curves and show results for mapping 
