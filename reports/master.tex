...
\begin{table}[H]
	...
	\begin{tabular}{lll}\toprule
		Layer                 & Activation Size & Value \\ \midrule
		Input                 & 1x28x28         &       \\
		linear layer          & 1x32x32         &       \\
		Dropout               & 1x32x32         & 0.15  \\
		LeakyReLU             & 1x32x32         & 0.2   \\
		linear layer          & 1x2048          &       \\
		Dropout               & 1x2048          & 0.15  \\
		LeakyReLU             & 1x2048          & 0.2   \\
		linear layer          & 1x32x32         &       \\
		Dropout               & 1x32x32         & 0.15  \\
		LeakyReLU             & 1x32x32         & 0.2   \\
		linear layer          & 1x512           &       \\
		Dropout               & 1x512           & 0.15  \\
		LeakyReLU             & 1x512           & 0.2   \\
		linear layer          & 1x16x16         &       \\
		Dropout               & 1x16x16         & 0.15  \\
		linear layer          & 1               &       \\
...
\begin{table}[H]
	...
	\begin{tabular}{lll}\toprule
		...
		InstanceNorm2d                                   &                 & 256         \\
		ReLU                                             &                 & inplace=True\\
		9 residual blocks with relu and instancenorm     & 256x64x64       &             \\
		Upsample                                         &                 & 2,bilinear  \\   
		128x4x4 Conv2d stride 1, pad 2                   & 128x128x128     &             \\
		InstanceNorm2d                                   &                 & 128         \\
		ReLU                                             &                 & inplace=True\\
		Upsample                                         &                 & 2,bilinear  \\   
		64x4x4 Conv2d stride 2, pad 1                    & 64x256x256      &             \\
		InstanceNorm2d                                   &                 & 64          \\
		ReLU                                             &                 & inplace=True\\
		3x8x8 conv, stride 1, pad 1                      & 3x256x256       &             \\
		Tanh                                             &                 & NA          \\ \bottomrule
	\end{tabular}
	\caption{The convolutions, stride and padding of each layer in the generators. The values used in the activation functions and instance normalization as well as when they are used in the network have also been shown. }
