...
def subject_images(sub):
	...
	for z in range(0,256):
		...
		io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/3T_imgs/xy_subset/{sub}_xy_z_{z}_3T.png', img_xy)
...
if __name__ == "__main__":
	...
	li = []
	f = open("/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/3T_imgs/ids_ADNI2_3T.txt", "r")
	for x in f:
  		li.append(x.strip())
	f.close()

	vali = []
	f = open("/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/3T_imgs/id_validation.txt")
	for x in f:
		vali.append(x.strip())
	f.close()
