...
def subject_images(sub):
    ...
    try:
        ...
        for z in range(0,256):
                ...
                if img_xz.sum() != 0:
                    io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/data/no_black/xz/train/B/B/{sub}_xy_z_{z}_3T.png', img_xz, check_contrast=False)
                img_yz = np.array(img[z,:,:]).astype(np.uint8)
                if img_yz.sum() != 0:
                    io.imsave(f'/dtu-compute/ADNIbias/AlzPred_Oskar_Anders/data/no_black/yz/train/B/B/{sub}_xy_z_{z}_3T.png', img_yz, check_contrast=False)
    except:
        pass 
