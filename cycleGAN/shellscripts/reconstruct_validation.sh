...
for f in $SUBJECT_NAMES; do 
	echo "Processing subject: $f"
    python reconstruct.py -p xy -s $f -t 1 -d /dtu-compute/ADNIbias/AlzPred_Oskar_Anders/git_code/AlzPred/oskar_implemetation/models/xy_nearest_lowlearning_final -a False
    echo "Done with: $f"
    echo ""
done
...
for f in $SUBJECT_NAMES; do 
	echo "Processing subject: $f"
    python reconstruct.py -p xz -s $f -t 1 -d /dtu-compute/ADNIbias/AlzPred_Oskar_Anders/git_code/AlzPred/oskar_implemetation/models/xz_nearest_lowlearning_final -a False
    echo "Done with: $f"
    echo ""
done
...
for f in $SUBJECT_NAMES; do 
	echo "Processing subject: $f"
    python reconstruct.py -p yz -s $f -t 1 -d /dtu-compute/ADNIbias/AlzPred_Oskar_Anders/git_code/AlzPred/oskar_implemetation/models/yz_nearest_lowlearning_final -a False
    echo "Done with: $f"
    echo ""
done