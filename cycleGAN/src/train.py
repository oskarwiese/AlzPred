...
def train_cykel(D_a, D_b, G_a, G_b, loader_a, loader_b, opti_D, opti_G, l1, mse, d_scaler, g_scaler, epoch, increment):
    ...
    for idx, ((a,_), (b,_)) in enumerate(loader):
        ...
        if idx % log_every == 0:
                log.log(f'loss_G_a: {loss_G_a} \nloss_G_b: {loss_G_b} \nloss_D_a: {D_a_loss} \nloss_D_b: {D_b_loss} \ncycle_loss_a: {cycle_a_loss} \ncycle_loss_b: {cycle_b_loss} \nidentity_loss_a: {identity_a_loss} \nidentity_loss_b: {identity_b_loss}')
                log.log(f'G_loss epoch_{epoch} idx_{idx}: {G_loss:.2f}\n\n\n')
                if args.dataset == "horse":
                    with torch.no_grad():
                        save_image(b*0.5 + 0.5, config.SAVE_IMG_DIR + f'/A/A_epoch_{epoch}_idx_{idx}_real_b.png')
                        save_image(a*0.5 + 0.5, config.SAVE_IMG_DIR + f'/B/B_epoch_{epoch}_idx_{idx}_real_a.png')
                        save_image(fake_b*0.5 + 0.5, config.SAVE_IMG_DIR + f'/B/B_epoch_{epoch}_idx_{idx}_fake_b.png')
                        save_image(fake_a*0.5 + 0.5, config.SAVE_IMG_DIR + f'/A/A_epoch_{epoch}_idx_{idx}_fake_a.png')
                else:
        ...
        if config.SAVE_MODEL:
            if epoch % save_every_epoch == 0 and idx == 0:
                save_checkpoint(G_a, opti_G, filename=f"{config.CHECKPOINT_GEN_A}_epoch_{epoch}_idx_{idx}.pth.tar")
                save_checkpoint(G_b, opti_G, filename=f"{config.CHECKPOINT_GEN_B}_epoch_{epoch}_idx_{idx}.pth.tar")
                save_checkpoint(D_a, opti_D, filename=f"{config.CHECKPOINT_DISC_A}_epoch_{epoch}_idx_{idx}.pth.tar")
                save_checkpoint(D_b, opti_D, filename=f"{config.CHECKPOINT_DISC_B}_epoch_{epoch}_idx_{idx}.pth.tar")
...
def main():
    ...
    if args.dataset == "horse":
        ...
        G_a = Generator(img_channels=3, num_residuals=9).to(config.DEVICE)  # Konverterer fra 3T->1.5T
        G_b = Generator(img_channels=3, num_residuals=9).to(config.DEVICE)  # Konverterer fra 1.5T->3T
    else:
...
if __name__ == '__main__':
    ...
    # Make sure to use the right generator
    if args.dataset == "horse":
